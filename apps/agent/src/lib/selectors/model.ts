...
export function buildSystemPrompt(stage: SelectorStage): string {
	...
	return `${shared} Task: identify sourcePanel and sourceItem. The sources UI is already open. sourcePanel: return a selector for EVERY distinct container that holds a source list for the latest answer. GROUPING RULE: when multiple sibling lists (ul, ol, div) exist inside a single parent container, return the PARENT container as sourcePanel — not the individual sibling lists. Only return individual list elements as separate sourcePanel entries when they are in genuinely separate UI regions (e.g. a side panel AND an inline tray). The goal is one sourcePanel selector per distinct UI region, where each selector encompasses all source items within that region. Some providers render citations in multiple separate locations; include a selector for each distinct region. Do not include the full document, page root, or top-level layout wrappers. sourceItem: the single most generic selector that matches all individual source cards, rows, or anchors across ALL panels — it must work inside every sourcePanel container. If items differ by panel, prefer the selector that matches the most items.`;
}
