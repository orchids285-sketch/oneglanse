...
function normalizeSelectorToken(token: string): string {
...
function isStableSemanticToken(token: string): boolean {
	...
	// Reject build-tool hash tokens: mixed case, short (≤10 chars), no separator
	if (
		/[A-Z]/.test(token) &&
		/[a-z]/.test(token) &&
		!/[-_]/.test(token) &&
...
function isStableSelectorValue(selector: string): boolean {
	...
	const snapshot = await page.evaluate((currentStage: SelectorStage) => {
		...
		function isStableSemanticToken(token: string): boolean {
			return (
				...
				!/__[a-z0-9]{5,}$/i.test(token) &&
				// Reject build-tool hash tokens: mixed case, short (≤10 chars), no separator
				// Keep in sync with module-scope isStableSemanticToken above
				!(/[A-Z]/.test(token) && /[a-z]/.test(token) && !/[-_]/.test(token) && token.length <= 10) &&
				(token.includes("-") ||
					token.includes("_") ||
...
function buildSystemPrompt(stage: SelectorStage): string {
	...
	const shared =
		...
		"NEVER use id or class tokens that are clearly build-tool generated: short (≤10 chars), mixed upper and lower case, with no hyphen or underscore (e.g. #APjFqb, .jloFI). Stable tokens are all-lowercase, use hyphens/underscores, or are recognisable camelCase library names (e.g. ProseMirror, CodeMirror). If the only selectors available are build-tool hash tokens, return []. " +
