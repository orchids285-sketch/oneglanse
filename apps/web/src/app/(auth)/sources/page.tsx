...
export default function SourcesPage(): React.JSX.Element {
  ...
  const metrics = useMemo<SourcePanelMetrics>(() => {
    ...
    const topDomainShare = totalCitations ? Math.round((topDomainCitations / totalCitations) * 100) : 0;
  ...
  const domainRows = useMemo<SourcePanelDomainRow[]>(
    () =>
      domainGroups.map((group) => ({
        domain: group.domain,
        share: metrics.totalCitations > 0 ? (group.totalCitations / metrics.totalCitations) * 100 : 0,
        ...
        <SectionHeading
          ...
          trailing={(
            ...
            <ExportMenu
              ...
              onExportCsv={() => {
                const rows = [
                  { section: "overview", metric: "Domains", value: metrics.totalDomains },
                  { section: "overview", metric: "URLs", value: metrics.totalUrls },
                  { section: "overview", metric: "Citations", value: metrics.totalCitations },
                  { section: "overview", metric: "Top Domain Share", value: `${metrics.topDomainShare}%` },
                  ...domainGroups.map((group) => ({
