...
If you're on Ubuntu 22.04 / 24.04, the setup script handles everything — dependencies, repo clone, `.env` config, nginx, SSL, and firewall — in one session:

```bash
# On your VPS (as a non-root user with sudo privileges):
curl -fsSL https://raw.githubusercontent.com/aryamantodkar/oneglanse/main/scripts/setup-vps.sh | bash
```
...
The script is interactive — it asks for your domain, LLM key, proxy URL, and auth token, then does the rest.
