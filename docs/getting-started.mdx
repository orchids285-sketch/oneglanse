...
If `BETTER_AUTH_SECRET` or `INTERNAL_CRON_SECRET` are still `replace-me`, `node scripts/run-compose.mjs bootstrap` generates them automatically before the containers start.
