...
**Symptom:** `node scripts/run-compose.mjs bootstrap` fails with a Docker pull error or `unauthorized`.

**Cause:** The GHCR images are unavailable or you're not authenticated with GHCR.

**Fix:** The VPS flow expects published images, not a local source build. Check outbound access to GHCR and Docker Hub, then retry:

```bash
node scripts/run-compose.mjs bootstrap
```

If you're maintaining a fork or testing unpublished changes, a local source build is still available as an advanced path via `pnpm self-host:build`, but that is not the standard VPS setup.
...
4. Check agent logs on the VPS: `docker logs oneglanse-agent-worker`
